...
mapboxgl.accessToken = 'pk.eyJ1Ijoicm5mZm0iLCJhIjoiY2wwNnJ2NnNhMDFubTNicW54ODRrb2R0aCJ9.PYIbDYazZScg-nQ4NBiKjg';
...
map.on('click', function (e) {
    ...
    if ((!event.ctrlKey && !event.shiftKey) || (event.ctrlKey && selected_objs.length == 0)) {
        ...
        if (id && id > -1) {
            ...
            // all_list.forEach( entitie => {
            //     if(entitie.type == "apartments"){
            //         entitie.polution = 300;
            //         entitie.range = 300;
            //     }
            // });
            createPropertiesTable(selection_obj, false);
...
function startAll() {
    ...
    all_list.forEach( entitie => {
        if(entitie.type == "apartments"){
            entitie.polution = 50;
            entitie.range = 50;
            // createPropertiesTable(entitie, false);
            // savePropsChanges();
        }
    });
    all_list.forEach( entitie => {
        if(entitie.type == "building"){
            entitie.polution = 100;
            entitie.range = 100;
            // createPropertiesTable(entitie, false);
            // savePropsChanges();
        }
    ...
    all_list.forEach( entitie => {
        if(entitie.type == "retail"){
            entitie.polution = 200;
            entitie.range = 200;
            // createPropertiesTable(entitie, false);
            // savePropsChanges();
        }
    ...
    all_list.forEach( entitie => {
        if(entitie.type == "commercial_area"){
            entitie.polution = 200;
            entitie.range = 200;
            // createPropertiesTable(entitie, false);
            // savePropsChanges();
    ...
    all_list.forEach( entitie => {
        if(entitie.type == "turning_circle"){
            entitie.polution = 300;
            entitie.range = 300;
            // createPropertiesTable(entitie, false);
            // savePropsChanges();
...
function savePropsChanges() {
    ...
    // console.log(extracted_props.profile.length)
    // console.log(extracted_props.profile[0]);
    // console.log(extracted_props.profile[1]);
    ...
    for (var i in selected_objs) {
        if (selected_objs[i].altered || true) altered = false;
        if (selected_objs[i].type != final_props.type ||
            selected_objs[i].polution != final_props.polution ||
            selected_objs[i].range != final_props.range || true) {
            altered = true;
        }
